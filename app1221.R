...
list.tb1 <- readRDS("tb1.RDS")
dinfo <- readRDS("dinfo.RDS")
...
label.dinfo <- c("PPI 30~ vs non-PPI" = 2, "PPI 90~ vs non-PPI" = 4, "PPI 180~ vs non-PPI" = 1, "PPI 365~ vs non-PPI" = 3)

# Define UI for application that draws a histogram
ui <- navbarPage(title = dinfo[[1]][[3]][[1]], 
                 tabPanel("Table 1",
                          sidebarLayout(
                            sidebarPanel(
                 ...
                 tabPanel("Kaplan-meier plot",
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              radioButtons("event_kap", "Event", choices = c('Event'), selected = "Event", inline = T)
                            ),
                            mainPanel(
...
server <- function(input, output) {
  ...
  output$info <- renderPrint({
    dinfo[[as.integer(input$Study)]][[3]]
  })
  ...
  output$tablecox <- renderDT({
    ...
    data <- data.cox()
    label <- dinfo[[as.integer(input$Study_cox)]][[4]]
  ...
  obj.km <- reactive({
    ...
    data <- data.kap()
    label <- dinfo[[as.integer(input$Study_kap)]][[4]]
    
    
    Makekaplan(event.original = input$event_kap, day.original = "Day",  var.group = "EXPCON", data = data, data.label = label,
               timeby = 365) 
