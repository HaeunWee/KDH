...
## 12/18 ARD 추가 Arcohol related disease
t14.pre.1 <- list(
  ARD =c('F10', nm('F10'), 'K70', nm('K70', 0, 43), 'Y91', nm('Y91', 0, 9), 
         "Z721", "K852", "K860", "M878", "I426", "G312", "G621","G312", "G405", "X65", "Z714", "Z502")
)
...
t14.sym <- t14[str_sub(SICK_SYM, 1, 3) %in% unique(unlist(t14.pre)) | SICK_SYM %in% unique(unlist(t14.pre.1))]
t14.sym <-
...
HTA <- function(x, y, z, tot){
  ...
  ## 위암연구의 과거력과 모양 동일
  vars.t14pre <- lapply(1:(length(t14.pre) + length(t14.pre.1)), function(i){
    if(i>length(t14.pre)){
      person.ev <- merge(TOT.temp[, .(PERSON_ID, indexdate)], t14.sym[SICK_SYM %in% t14.pre.1[[(i-length(t14.pre))]]], by = "PERSON_ID")[order(PERSON_ID, RECU_FR_DT)][, .SD[1], keyby = "PERSON_ID"][RECU_FR_DT <= indexdate][, .(PERSON_ID, var = 1)]
  ...
  colnames(vars.t14pre) <- paste0("Pre_", union(names(t14.pre),names(t14.pre.1)))
...
l <- ALLINONE(c('dementia','vasculardementia','cerebralinfarction', 'osteoporosis', 'ovf', 'ohf'), c(0, 30, 90, 180, 365), 365, 180, 30, hra = FALSE)
ppi30ppi0 <- as.data.table(l[[1]][[1]])
ppi90ppi0 <- as.data.table(l[[1]][[2]])
ppi180ppi0 <- as.data.table(l[[1]][[3]])
ppi365ppi0 <- as.data.table(l[[1]][[4]])
fwrite(df(ppi30ppi0), "/home/whe/period30/1218/300.csv")
fwrite(df(ppi90ppi0), "/home/whe/period30/1218/900.csv")
fwrite(df(ppi180ppi0), "/home/whe/period30/1218/1800.csv")
fwrite(df(ppi365ppi0), "/home/whe/period30/1218/3650.csv")
print("End")
